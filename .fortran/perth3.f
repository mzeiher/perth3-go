...
*     3.2    7/23/09     Remko    Fixed old bug in GRSINT, which PERTH3 calls.
*
*
Cf2py intent(in)    DLAT          ! f2py preprocessor directive added by ASCH 31 May 2016
Cf2py intent(in)    DLON          ! f2py preprocessor directive added by ASCH 31 May 2016
Cf2py intent(in)    TIME          ! f2py preprocessor directive added by ASCH 31 May 2016 
Cf2py intent(inout) TIDE          ! f2py preprocessor directive added by ASCH 31 May 2016
Cf2py intent(inout) ISDATA        ! f2py preprocessor directive added by ASCH 31 May 2016
     
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DOUBLE PRECISION  NINETY
      PARAMETER (MX=2881, MY=1441, NG=10, NT=28)
...
*     -----------------------------------------------
      IF (INIT) THEN
         INIT = .FALSE.
         OPEN(LU, FILE="tidal_constituents.dat")  ! avoid file fort.30 - ASCH 31 May 2016
...
*
      ...
      PARAMETER (NT=10)
      LOGICAL    SELECT(NT)
      PARAMETER (D2R=1.745329 E-2,  MXX=2881,MXY=1441)
...
*
      ...
      READ(LU,*,END=101) TITLE
      read(LU,*)
c     READ(LU,1) TITLE(81:160)
    1 FORMAT(A30)
    ...
    2 FORMAT(16X,I5,16X,I5)
      IF (NX.GT.NDIM1 .OR. NY.GT.NDIM2) THEN
         ...
         STOP
      ENDIF
      READ(LU,*) LATMIN,LATMAX
      READ(LU,*) LONMIN,LONMAX
    3 FORMAT(16X,F9.0,16X,F9.0)
      READ(LU,*) UNDEF           ! 2 masks may be here
      READ(LU,*) FORMAT
c      print *, TITLE                           ! uncommented by ASCH 22 Jan 2016
c      print *, NY,NX                           ! uncommented by ASCH 22 Jan 2016
c      print *, LATMIN,LATMAX,LONMIN,LONMAX     ! uncommented by ASCH 22 Jan 2016
c      print *, UNDEF                           ! uncommented by ASCH 22 Jan 2016
c      print *, FORMAT                          ! uncommented by ASCH 22 Jan 2016
      DO 4 J=1,NY
...
! Changed by Remko Scharroo, because this resulted in errors for points
! between -0.5..0.0 or 359.5..360.0
      IF (WRAP.AND.XLON.GE.LONMIN+360.0) XLON = XLON - 360.0
