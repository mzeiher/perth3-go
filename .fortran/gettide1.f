...
c Test driver for perth3
      implicit none 
      double precision dlat,dlon,time,tide,lptide,aobs,aobs1
      integer*4 npoints,x,ipoint,ifirst,ilast,istep
      integer*2 ersfile,nnum
      logical  isdata
      character*70 foname,headnm,fname,ffname 

        write(*,*) 'Enter Output filename'
        read(*,'(A)') foname
        npoints  = 40000000
        write(*,*) 'Enter first time:Seconds since 1985:01:01 00:00 '
        read (*,*) ifirst
        write(*,*) 'Enter last time:Seconds since 1985:01:01 00:00 '
        read (*,*) ilast
        write(*,*) 'Enter time step (seconds)'
        read (*,*) istep
        write(*,*) 'Enter location (latitude, longitude) '
        read (*,*) dlat,dlon

	open(61,file=foname) 
        npoints = (ilast-ifirst)/istep
        write(*,*) 'starting (lat,long,ipoints)',dlat,dlon,npoints
        
        do ipoint =1,npoints 
      !     if (mod(ipoint,25000).eq.1) write(*,*) ipoint
c         get time in days
            time = ifirst + (ipoint-1)*istep
c         convert time (seconds since 01:01:1985 into MJD
            time = time/(86400.d0)+(46066.d0)
            write(*,*) 'time',time
            ...
            tide = tide + lptide
            write(*,*) 'tide2',tide
c           write(61,*) x,dlat,dlon,aobs-tide/100.,aobs1
	    else
	      tide = 0.000
          endif 
          write(61,*) (time-46066),tide/100.

11        format(F12.5,2f8.2,f8.2,6x,L1)
12        format(5x,'time',7x,'lat',5x,'lon',5x,'tide')
101       format(i11,2d11.5,d10.3,d10.1)
        end do
9001    continue
        close(61)
       stop
...
*
      ...
      PARAMETER (MX=5761, MY=2881, NG=10, NT=28)
...
*
      ...
      PARAMETER (NT=10)
      LOGICAL    SELECT(NT)
      PARAMETER (D2R=1.745329 E-2,  MXX=5761,MXY=2881)
...
*
    ...
    2 FORMAT(16X,I5,16X,I5)
      IF (NX.GT.NDIM1 .OR. NY.GT.NDIM2) THEN
         WRITE(6,*) 'NX NDIM1 NY NDIM2 ',nx,ndim1,ny,ndim2
